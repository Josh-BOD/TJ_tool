"""
TrafficJunky Multi-Format Ad Automation Tool - Main Entry Point

Automates bulk ad creative uploads to TrafficJunky campaigns.
Supports both NATIVE and INSTREAM (Preroll) ad formats.
"""
...
from src.native_uploader import NativeUploader
from src.uploader import TJUploader  # For instream/preroll
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='TrafficJunky Multi-Format Ad Automation Tool - Upload Native or In-Stream ads via CSV',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Dry run (default - no actual uploads) - Native ads
  python multi_format_main.py --ad-format NATIVE
  
  # Live mode (actual uploads) - In-Stream ads
  python multi_format_main.py --live --ad-format INSTREAM
  
  # Process specific campaigns
  python multi_format_main.py --campaigns 1013017411,1013017412 --ad-format NATIVE
  
  # Headless mode (no browser window)
  python multi_format_main.py --headless --ad-format INSTREAM

Supported Ad Formats:
  NATIVE - Native ad format (requires video, thumbnail, headline, brand)
  INSTREAM - In-stream/Preroll video ads (requires video creative, CTA)
        """
    )
    ...
    parser.add_argument(
        '--ad-format',
        type=str,
        choices=['NATIVE', 'INSTREAM'],
        required=True,
        help='Ad format to upload: NATIVE or INSTREAM'
    )
...
def main():
    ...
    checkpoint_exists = (Config.BASE_DIR / 'data' / 'session' / 'native_upload_checkpoint.json').exists()
    ...
    # Print banner
    print_banner()
    ad_format = args.ad_format.upper()
    if ad_format == "NATIVE":
        print_info("🎯 NATIVE AD UPLOADER MODE")
    else:
        print_info("🎬 IN-STREAM AD UPLOADER MODE")
    print("")
    
    # Setup logging
    log_level = 'DEBUG' if args.verbose else Config.LOG_LEVEL
    log_file = Config.LOG_DIR / f"native_upload_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    logger = setup_logger('native_main', log_file, log_level, Config.LOG_TO_CONSOLE, Config.LOG_TO_FILE)
    
    logger.info("="*60)
    logger.info("TrafficJunky Native Ad Automation Tool Started")
    logger.info("="*60)
    ...
    logger.info("Loading Native campaigns...")
    # Look for native_campaign_mapping.csv first, fall back to campaign_mapping.csv
    native_mapping = Config.CSV_INPUT_DIR / 'native_campaign_mapping.csv'
    if not native_mapping.exists():
        logger.warning("native_campaign_mapping.csv not found, using campaign_mapping.csv")
        native_mapping = Config.CSV_INPUT_DIR / 'campaign_mapping.csv'
    
    checkpoint_file = Config.BASE_DIR / 'data' / 'session' / 'native_upload_checkpoint.json'
    campaign_manager = CampaignManager(
        mapping_file=native_mapping,
    ...
    print_success(f"Loaded {len(campaign_manager.campaigns)} Native campaigns")
    
    # Handle checkpoint
    session_id = f"native_upload_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    try:
        with sync_playwright() as p:
            ...
            # Initialize uploader based on ad format
            if ad_format == "NATIVE":
                uploader = NativeUploader(
                    dry_run=dry_run,
                    take_screenshots=take_screenshots
                )
                print_info("Using Native Ad Uploader")
            else:  # INSTREAM
                uploader = TJUploader(
                    dry_run=dry_run,
                    take_screenshots=take_screenshots
                )
                print_info("Using In-Stream/Preroll Ad Uploader")
            ...
            logger.info(f"Starting upload of {total_campaigns} {ad_format} campaigns...")
            
            # Create progress bar
            with tqdm(
                total=total_campaigns,
                desc=f"Uploading {ad_format} campaigns",
                unit="campaign",
                ...
                while campaign:
                    ...
                    logger.info(f"\n{'='*60}")
                    logger.info(f"Processing {ad_format} campaign: {campaign.campaign_id} ({campaign.campaign_name})")
                    logger.info(f"{'='*60}")
                    
                    # Get CSV path
                    csv_path = campaign_manager.get_csv_path(campaign)
                    
                    # Validate CSV (Native format)
                    is_valid, errors = NativeCSVProcessor.validate_csv(csv_path)
                    if not is_valid:
                    ...
                    logger.info("Navigating to Native campaign to get actual name from TJ...")
                    ...
                    csv_path = NativeCSVProcessor.update_campaign_name_in_urls(csv_path, tj_campaign_name, Config.WIP_DIR)
                    
                    logger.info(f"Updated CSV path: {csv_path}")
                    logger.info(f"Dry run mode: {dry_run}")
                    logger.info("Starting upload to campaign...")
                    ...
                    if result['status'] == 'success':
                        ...
                        try:
                            cleaned_csv, removed_rows = NativeCSVProcessor.remove_invalid_creatives(
                                csv_path,
            ...
            # Delete session for security (fresh login each time)
            try:
                session_file = Config.BASE_DIR / 'data' / 'session' / 'tj_session.json'
                if session_file.exists():
                    session_file.unlink()
                    logger.info("✓ Session deleted (fresh login required next time)")
            except Exception as e:
                logger.warning(f"Could not delete session: {e}")
            
    except KeyboardInterrupt:
        print_warning("\nProcess interrupted by user")
        # Delete session even on interrupt
        try:
            session_file = Config.BASE_DIR / 'data' / 'session' / 'tj_session.json'
            if session_file.exists():
                session_file.unlink()
    ...
    print_success(f"Native Ad upload process completed in {format_duration(duration)}")
    
    # Offer to clean up WIP folder
    cleanup_wip_folder()
    
    logger.info("="*60)
    logger.info("TrafficJunky Native Ad Automation Tool Finished")
    logger.info("="*60)

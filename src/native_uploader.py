"""Native ad CSV upload automation module."""
...
class NativeUploader:
    """Handles Native ad CSV upload to TrafficJunky campaigns."""
    ...
    def upload_to_campaign(
        ...
        """
        Upload Native ad CSV to a specific campaign.
        
        Args:
            page: Playwright page object
            campaign_id: TrafficJunky campaign ID
            csv_path: Path to Native CSV file
            screenshot_dir: Directory to save screenshots
            skip_navigation: If True, skip navigation (already on campaign page)
            
        Returns:
            Dictionary with upload results
        """
        logger.info(f"="*60)
        logger.info(f"NativeUploader.upload_to_campaign() CALLED")
        logger.info(f"Campaign ID: {campaign_id}")
        logger.info(f"CSV Path: {csv_path}")
        logger.info(f"Dry Run: {self.dry_run}")
        logger.info(f"Skip Navigation: {skip_navigation}")
        logger.info(f"="*60)
        ...
        try:
            logger.info(f"Processing Native campaign {campaign_id}...")
            ...
            self._take_screenshot(page, f"01_native_campaign_{campaign_id}_loaded", screenshot_dir)
            ...
            logger.info(f"Native campaign has {existing_ads} existing ads")
            ...
            # Step 5: Upload CSV
            if self.dry_run:
                logger.info(f"DRY RUN: Would upload Native CSV {csv_path}")
                result['status'] = 'dry_run_success'
                return result
            
            if not self._upload_csv_file(page, csv_path):
                result['error'] = "Failed to upload Native CSV file"
                return result
            
            self._take_screenshot(page, f"03_native_csv_uploaded", screenshot_dir)
            ...
            if self._has_validation_errors(page):
                logger.warning("Validation errors detected in Native upload")
                ...
                self._take_screenshot(page, f"04_native_validation_errors", screenshot_dir)
            ...
            self._take_screenshot(page, f"05_native_preview_created", screenshot_dir)
            
            # Step 9: Click create ads
            if not self._click_create_ads(page):
                result['error'] = "Failed to click create Native ads button"
                return result
            
            # Step 10: Wait for processing and reload to get fresh ad count
            logger.info("Waiting for Native ads to be created...")
            time.sleep(5)  # Wait for processing
            
            self._take_screenshot(page, f"06_native_ads_created", screenshot_dir)
            
            # Reload the page to get fresh ad count
            logger.info("Reloading page to verify Native ad creation...")
            ...
            logger.info(f"Native ad count - Before: {existing_ads}, After: {new_ad_count}, Created: {ads_created}")
            
            if ads_created > 0:
                logger.info(f"✓ Successfully created {ads_created} Native ads")
                ...
                logger.warning(f"Could not detect new Native ads, but found {new_ad_count} total ads in campaign")
                logger.warning("Upload likely succeeded (creative may have already existed)")
                result['status'] = 'success'
                result['ads_created'] = 1  # Assume 1 ad was uploaded
            else:
                logger.warning("No new Native ads detected")
                result['error'] = "No new Native ads created"
            
            return result
            
        except Exception as e:
            logger.error(f"✗ Native upload failed for campaign {campaign_id}: {e}")
            result['error'] = str(e)
            self._take_screenshot(page, f"ERROR_native_campaign_{campaign_id}", screenshot_dir)
            return result
    ...
    def _navigate_to_campaign(self, page: Page, campaign_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Navigating to Native campaign: {url}")
            
            page.goto(url, wait_until='networkidle', timeout=30000)
            
            # Wait for page to load - try multiple possible selectors for Native campaigns
            try:
                # Try the standard ad creation text first
                page.wait_for_selector('text=STEP 5. CREATE YOUR AD', state='visible', timeout=5000)
                logger.debug("Found 'STEP 5. CREATE YOUR AD' on page")
            except:
                # For Native campaigns, the text might be different
                # Just wait for the page to be loaded (URL should be correct)
                logger.info("Standard text not found, checking for Native ad specific elements...")
                try:
                    # Wait for any ad-related selector (mass create, table, etc.)
                    page.wait_for_selector('text=Mass create with CSV', state='visible', timeout=5000)
                    logger.debug("Found 'Mass create with CSV' - Native campaign detected")
                except:
                    # Last resort: just check if we're on the right URL
                    current_url = page.url
                    if 'ad-settings' in current_url and campaign_id in current_url:
                        logger.info("On ad-settings page, proceeding...")
                    else:
                        raise Exception(f"Could not verify ad settings page. Current URL: {current_url}")
            
            return True
        except Exception as e:
            logger.error(f"Navigation to Native campaign failed: {e}")
            return False
                ...
                try:
                    ...
                    if element.is_visible(timeout=2000):
                        ...
                        if text and len(text) > 5 and 'STEP' not in text:
                            logger.info(f"✓ Found Native campaign name from TJ: {text}")
                            return text.strip()
                except:
                    continue
            
            logger.warning("Could not extract Native campaign name from page")
            return ""
            
        except Exception as e:
            logger.warning(f"Error extracting Native campaign name: {e}")
            return ""
    ...
    def _count_existing_ads(self, page: Page) -> int:
        ...
        try:
            ...
            logger.debug(f"Found {table_count} tables on Native campaign page")
            ...
            for i in range(table_count):
                ...
                if table.is_visible():
                    ...
                    if rows > 0:  # Found table with data
                        logger.debug(f"Found Native ads table with {rows} rows")
                        return max(0, rows)
            
            logger.debug("No Native ads table found or table is empty")
            return 0
        except Exception as e:
            logger.warning(f"Error counting Native ads: {e}")
            return 0
    ...
    def _set_ads_page_length(self, page: Page, length: int = 100) -> bool:
        ...
        """
        try:
            logger.info(f"Setting Native ads table page length to {length}...")
            
            # Look for the page length dropdown (similar to campaigns page)
            # The selector might be different, so try multiple options
            selectors = [
                'span#select2-dt-length-1-container',  # Based on Preroll implementation
            ...
            for selector in selectors:
                try:
                    ...
                    if dropdown.is_visible(timeout=2000):
                        ...
                        logger.debug(f"Clicked Native ads page length dropdown with selector: {selector}")
                        time.sleep(1)
                        break
                except:
                    continue
            ...
            if not dropdown_clicked:
                logger.debug("Native ads page length dropdown not found, may not be needed")
                return False
            ...
            try:
                ...
                option.click()
                logger.info(f"✓ Set Native ads page length to {length}")
                ...
                try:
                    ...
                    option.click()
                    logger.info("✓ Set Native ads page length to show All ads")
                    time.sleep(2)
                    return True
                except:
                    logger.warning("Could not find Native ads page length option")
                    return False
                    
        except Exception as e:
            logger.debug(f"Could not set Native ads page length: {e}")
            return False
    ...
    def _select_mass_csv(self, page: Page) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to select Mass CSV for Native ads: {e}")
            return False
    
    def _upload_csv_file(self, page: Page, csv_path: Path) -> bool:
        """Upload Native CSV file."""
        try:
            logger.info(f"Uploading Native CSV: {csv_path}")
            
            # Use set_input_files on the file input
            file_input = page.locator('#massAdsCsv')
            file_input.set_input_files(str(csv_path))
            
            logger.info("✓ Native CSV file uploaded")
            return True
        except Exception as e:
            logger.error(f"Failed to upload Native CSV: {e}")
            return False
    ...
    def _extract_invalid_creatives(self, page: Page) -> List[str]:
        ...
        try:
            ...
            if error_element.is_visible():
                ...
                logger.warning(f"Found {len(creative_ids)} invalid creative IDs in Native upload: {creative_ids}")
                return creative_ids
        except Exception as e:
            logger.debug(f"Could not extract invalid creatives from Native upload: {e}")
